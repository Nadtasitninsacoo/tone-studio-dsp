  ...
  if (a.maxMasterDb <= CEILING_DB + CEILING_TOLERANCE_DB) {
    ...
    if (overshoot > 0.05) {
      lines.push(
        ...
        'is not rounding.',
        '',
        'What is already ruled out, by measurement rather than by reading:',
        '  · `setCeiling` and its dB→linear conversion — moving the ceiling 6 dB moved the',
        '    overshoot 0.2 dB, and a scaling error would have doubled it;',
        '  · the meter — `Metering.cpp` measures true peak through a 4× FIR interpolator',
        '    now, having been `max(abs(x))` at base rate under a true-peak name;',
        '  · the limiter reading the signal worse than the meter does — both reconstruct',
        '    through `filterHalfBandFIREquiripple` at 4×. Switching the limiter off the',
        '    polyphase IIR took a real guitar from 0.7 dB over to 0.4.',
        '',
        'What is left is the hard clamp after `processSamplesDown`. It holds every *sample*',
        'to the ceiling, which is a real guarantee and a different one: flattening the tops',
        'adds harmonics, and harmonics are inter-sample peaks. The stage added to enforce',
        'the ceiling is the current suspect for the true peak still crossing it.',
        '',
        'To test that, raise `kDownsampleHeadroomDb` until the clamp stops firing and run',
        'this again. If the overshoot goes with it, the clamp was making it.',
      );
    }
